...
@SpringBootTest(properties = {
        "spring.datasource.url=jdbc:h2:mem:kairo_platform_web_contract;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DEFAULT_NULL_ORDERING=HIGH"
})
...
class PlatformWebContractIntegrationTest {
    ...
    void registersRuntimeAssignsEnvironmentAndDiscoversExactJvmTargets() throws Exception {
        JsonNode registration = postJson("/api/v1/agent-registrations/self", Map.ofEntries(
                Map.entry("projectName", "kairo"),
                Map.entry("applicationName", "kairo-demo"),
        ...
        assertThat(registration.path("projectName").asText()).isEqualTo("kairo");
        assertThat(registration.path("applicationName").asText()).isEqualTo("kairo-demo");
        ...
        mockMvc.perform(get("/api/v1/query/instances")
                        .header("X-Actor", "system"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.items[0].project_name").value("kairo"))
                .andExpect(jsonPath("$.items[0].application_name").value("kairo-demo"))
    ...
    void registersRuntimeWithEnvironmentNameAsAssignedInstance() throws Exception {
        JsonNode registration = postJson("/api/v1/agent-registrations/self", Map.ofEntries(
                Map.entry("projectName", "kairo"),
                Map.entry("applicationName", "kairo-demo-env-name"),
