...
#
# V1.7 M2-B state-cycle harness runner (§9.2 / §9.5). Runs real Agent/JVM
# lifecycle cycles distributed across the fixed six-scenario matrix and writes
# target/<output>/state-cycle-result.json.
#
# Fixed interface (§9.5):
#   ./scripts/v1.7/run-state-cycle.sh --cycles 500 --output target/v1.7
#
# A cycle is one complete lifecycle on one real target: enhance via real Byte
# Buddy / AgentRuntime, invoke and verify behaviour, update or partially unload,
# invoke and verify again, fully unload, invoke and verify the original behaviour,
# and prove the normalized bytecode hash returned to the pre-enhancement baseline.
#
# Exit codes (preserved exactly from the harness):
#   0  all cycles passed and schema valid
#   1  usage / validation error (incl. dirty PR tree)
#   2  build failed
#   3  harness unusable
#   4  lifecycle failure (an invalid lifecycle; firstFailure recorded in the result)
...
# harness in place (HEAD), runs it in a fresh JVM with fixed JVM args, preserves
# the exact harness exit code, and cleans only this runner's prior state-cycle
# output. It never modifies budgets or the acceptance manifest.

set -euo pipefail
# Fail-fast is on (`-e`). The ONLY intentional non-fatal command is the harness
# invocation: it returns 4/5/6 on lifecycle/write/schema failure, and we must
# capture that status explicitly and exit with the EXACT code (see the `set +e`
# block around the harness below) rather than abort early. Every other failing
# command (build / classpath) aborts immediately so it can never silently continue.

CYCLES=""
OUTPUT=""
JVM_ARGS="${KAIRO_STATECYCLE_JVM_ARGS:--Xms512m -Xmx512m -XX:+AlwaysPreTouch}"
ALLOW_DIRTY="false"
...
HARNESS_MAIN="com.example.kairo.perf.statecycle.StateCycleHarness"
...
usage() {
  cat <<'EOF'
Usage: run-state-cycle.sh --cycles <N> --output <dir> [--jvm-args <args>]
                          [--allow-dirty] [--help]

Required:
  --cycles       total cycles to distribute across the scenario matrix (>= 6, so
                 every scenario runs at least once; NOT per-scenario repetitions)
  --output       output directory for state-cycle-result.json (e.g. target/v1.7)
...
Behavior:
  ...
  - Cleans only this runner's prior state-cycle-result.json before running.
  - Runs the harness in a fresh JVM with fixed JVM args; the exact harness exit
    code is preserved.
  - The harness writes target/<output>/state-cycle-result.json even on failure
...
EXTDEPS_FILE="$(mktemp -t kairo-statecycle-deps-XXXXXX)"
...
echo "==> running state-cycle harness"
# The harness returns 4/5/6 on lifecycle/write/schema failure. Temporarily disable
...
echo "==> done (harness exit=$HARNESS_STATUS). result: $OUTPUT_DIR/state-cycle-result.json"
exit "$HARNESS_STATUS"
