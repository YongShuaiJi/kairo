...
const templates = {
  ...
  setResult: `result.status = 'REVIEW_REQUIRED'\nresult.message = 'changed by runtime mock'\nreturn mock.returnValue(result)`,
  throwsReturn: `return mock.returnJson('''\n{\n  "id": "DEGRADED",\n  "status": "DEGRADED",\n  "amount": 0,\n  "message": "fallback"\n}\n''')`
};

function api(path, options = {}) {
  const url = `/api${path}${path.includes("?") ? "&" : "?"}agent=${encodeURIComponent(state.agent)}&token=${encodeURIComponent(state.token)}`;
  return fetch(url, {
    ...options,
    headers: {
...
$("connectionForm").onsubmit = async (event) => {
  event.preventDefault();
  state.agent = $("agentUrl").value.trim();
