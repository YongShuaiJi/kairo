...
/**
 * Pure argument parser for {@link LeakCheckHarness}. Mirrors the shell-side validation
 * so the harness fails fast with a precise error when invoked directly. The shell runner
 * ({@code run-leak-check.sh}) performs the same checks before launching the JVM; this
 * parser is the in-process authority for tests and direct invocation.
 *
 * <p>Recognised flags:
 * <ul>
 *   <li>{@code --cycles <N>}   total cycles to distribute across the matrix (>= {@value LeakScenarioCatalog#MIN_CYCLES})</li>
 *   <li>{@code --output <dir>} output directory for {@code leak-result.json}</li>
 ...
 * </ul>
 */
public final class LeakArgumentParser {
    ...
    public static Options parse(String... args) {
        ...
        if (cycles < LeakScenarioCatalog.MIN_CYCLES) {
            throw new IllegalArgumentException(
                    "--cycles must be >= " + LeakScenarioCatalog.MIN_CYCLES
