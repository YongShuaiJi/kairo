...
function sessionKey() {
  const secret = process.env.KAIRO_WEB_SESSION_KEY;
  if (!secret || secret.length < 32) {
    if (process.env.KAIRO_WEB_DEMO_MODE === "true") {
      return createHash("sha256").update("kairo-demo-session-key-only").digest();
    }
    throw new Error("KAIRO_WEB_SESSION_KEY must contain at least 32 characters");
  }
